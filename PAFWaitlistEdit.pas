...
// *********************************************************************
// PafWaitlistEdit är en descendant av TCustomItemEditor
...
type
  ...
  TPAFWaitlistItemEditor = class(TCustomItemEditor)
  private
...
procedure Register;
begin
  RegisterComponents('PAF-TMS', [TPAFWaitlistItemEditor]);
end;
...
begin
  ...
  with FEditForm do
  begin
    Panel1.Caption.Text:=TMyPafItem(APlannerItem).Prodkod;
    ...
    {$ifdef edit_debug}
    EditKommentar.text := 'Alnr: '+IntToStr(TMyPafItem(APlannerItem).ALNr)+
                          '.  StatusFlg: ' + IntToStr(TMyPafItem(APlannerItem).StatusFlg) ;
    {$else}
